...
const UserMachineRenterCard = (props) => {
  ...
  const toggleSidebar = async () => {
    ...
    const activerent = await axios.post(
      ...
      `${process.env.REACT_APP_BACKEND_URI}/api/requests/getactiverental`,
      { machineid, token }
    );
  ...
  const handlePay = async () => {
    ...
    try {
      const response = await axios.post(
        // "http://localhost:4000/api/requests/rentclose",
        `${process.env.REACT_APP_BACKEND_URI}/api/requests/rentclose`,
  ...
  return (
    <>
      ...
      <div className="flex z-70 ">
        ...
        {/* Sidebar */}
        <div
          className={`fixed inset-y-0 right-0 w-[20vw] shadow-2xl bg-white transform ${
